...
public interface ParametresRepository extends JpaRepository<Parametres, Integer> {
       ...
       /**
        * Rechercher les paramètres collectés par des dispositifs créés par un
        * administrateur spécifique
        */
       @Query("SELECT p FROM Parametres p WHERE p.dispositif.administrateur.id = :adminId")
       List<Parametres> findAllByAdministrateurId(@Param("adminId") Integer adminId);

       /**
        * Compter les patients uniques pour un admin
        */
